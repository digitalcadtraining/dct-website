...
async function calculateRevenue(where = {}) {
  ...
  return paidEnrollments.reduce((sum, item) => sum + toNumber(item.batch?.course?.price), 0);
}
...
const getStats = async (req, res, next) => {
  try {
    ...
    const totalBatches = activeBatches + completedBatches + upcomingBatches + pendingBatches;
    return success(res, 200, "Platform stats.", {
      totalStudents, activeStudents, totalTutors, pendingApplications,
      activeBatches, completedBatches, upcomingBatches, pendingBatches, totalBatches,
      totalQueries, unresolvedQueries, resolvedQueries, paidEnrollments, totalRevenue, monthRevenue,
      health: {
  ...
  try {
    ...
    const [applications, total] = await Promise.all([
      prisma.tutorApplication.findMany({
        where, skip, take, orderBy: { applied_on: "desc" },
        include: {
...
const approveApplication = async (req, res, next) => {
  try {
    const { id } = req.params;
    const application = await prisma.tutorApplication.findUnique({ where: { id }, include: { course: true } });
    ...
    const { tutorUser } = await prisma.$transaction(async (tx) => {
      const tutorUser = await tx.user.create({
        data: { name: application.name, email: application.email, phone: application.phone, password_hash, role: "TUTOR", is_verified: true, is_active: true },
      });
      await tx.tutorApplication.update({ where: { id }, data: { status: "APPROVED", reviewed_on: new Date(), user_id: tutorUser.id } });
      return { tutorUser };
    });
    ...
    return success(res, 200, "Application approved. Tutor account created and credentials sent.", {
      tutor_id: tutorUser.id, name: application.name, email: application.email,
...
const rejectApplication = async (req, res, next) => {
  try {
    ...
    if (!application) return error(res, 404, "Application not found.");
    if (application.status !== "PENDING") return error(res, 400, "Application already reviewed.");
    await prisma.tutorApplication.update({ where: { id: req.params.id }, data: { status: "REJECTED", reviewed_on: new Date(), rejection_note } });
...
const listStudents = async (req, res, next) => {
  try {
    ...
    const { skip, take, page: p, pageSize: ps } = getPagination(page, pageSize || 50);
    const where = {
      role: "STUDENT",
      ...(search && { OR: [
    ...
    const [students, total] = await Promise.all([
      prisma.user.findMany({
        ...
        select: {
          id: true, name: true, email: true, phone: true, is_active: true, created_at: true,
          enrollments: {
            orderBy: { enrolled_at: "desc" },
            include: { batch: { select: { id: true, name: true, start_date: true, end_date: true, status: true, course: { select: { name: true } }, tutor: { select: { name: true } } } } },
          },
        },
      }),
...
const listTutors = async (req, res, next) => {
  try {
    const tutors = await prisma.user.findMany({
      where: { role: "TUTOR" }, orderBy: { created_at: "desc" },
      select: {
        id: true, name: true, email: true, phone: true, is_active: true, created_at: true,
        tutor_application: { select: { course: { select: { name: true } }, years_exp: true, location: true, occupation: true } },
        tutor_batches: { select: { id: true, status: true, _count: { select: { enrollments: true, scheduled_sessions: true } } } },
      },
    });
  ...
  try {
    ...
    const queries = await prisma.query.findMany({
      ...
      include: {
        ...
        batch: { select: { name: true, course: { select: { name: true } }, tutor: { select: { name: true } } } },
      },
    });
...
const resolveQuery = async (req, res, next) => {
  try {
    ...
    const updated = await prisma.query.update({
      ...
      data: { status: "RESOLVED", answered_at: q.answered_at || new Date(), answer: q.answer || "Marked resolved by admin." },
      include: { student: { select: { name: true, email: true } }, session: { select: { name: true, session_number: true } }, batch: { select: { name: true } } },
    });
...
const approveBatch = async (req, res, next) => {
  try {
    const updated = await prisma.batch.update({ where: { id: req.params.id }, data: { status: "UPCOMING" } });
