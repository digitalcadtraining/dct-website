...
function withSlotTime(dateValue, slot) {
  ...
  if (slotStart) d.setHours(slotStart.h, slotStart.min, 0, 0);
...
function projectToStudent(project) {
  const meta = projectMeta(project);
  return { id: project.id, name: project.name, is_recorded: isRecordedProject(project), delivery_mode: isRecordedProject(project) ? "RECORDED" : "LIVE", unlock_rule: meta.unlock_rule || (isRecordedProject(project) ? "FIRST_LIVE_PROJECT_START" : null) };
}
...
function buildLiveSessionItems(syllabusSessions = [], syllabusProjects = []) {
  ...
  for (const project of syllabusProjects) {
    liveProjectSessions(project).forEach((s) => projectItems.push({ name: `Project: ${project.name} - ${s.name}`, type: "CAD" }));
  }
...
async function getTutorCourseProjectMap(pairs) {
  ...
  const applications = await prisma.tutorApplication.findMany({ where: { status: "APPROVED", OR: uniquePairs.map((p) => ({ user_id: p.tutor_id, course_id: p.course_id })) }, include: { syllabus_projects: true } });
  const map = new Map();
...
function safeSessionStatus(value) {
  ...
  return allowed.has(String(value || "").toUpperCase()) ? String(value).toUpperCase() : "UPCOMING";
}
function safeSessionType(value) {
  const allowed = new Set(["THEORY", "CAD", "BOTH"]);
  return allowed.has(String(value || "").toUpperCase()) ? String(value).toUpperCase() : "BOTH";
}
function safeBatchStatus(value, current = "UPCOMING") {
  const allowed = new Set(["PENDING_APPROVAL", "UPCOMING", "ACTIVE", "COMPLETED"]);
  return allowed.has(String(value || "").toUpperCase()) ? String(value).toUpperCase() : current;
}
...
const createBatch = async (req, res, next) => {
  try {
    ...
    const normalizedSlots = Array.isArray(time_slots) ? time_slots.filter(Boolean) : typeof time_slots === "string" && time_slots.trim() ? [time_slots.trim()] : [];
    if (normalizedSlots.length === 0) return error(res, 400, "At least one class time slot is required.");
    const application = await prisma.tutorApplication.findFirst({ where: { user_id: tutorId, course_id, status: "APPROVED" }, include: { syllabus_sessions: { orderBy: { session_number: "asc" } }, syllabus_projects: true, course: { select: { name: true, short_name: true } } } });
    ...
    const endDate = sessionDates.length > 0 ? sessionDates[sessionDates.length - 1] : new Date(new Date(start_date).setMonth(new Date(start_date).getMonth() + 4));
    ...
    const batch = await prisma.$transaction(async (tx) => {
      const newBatch = await tx.batch.create({ data: { course_id, tutor_id: tutorId, name: batchName, start_date: withSlotTime(start_date, mainSlot) || new Date(start_date), end_date: withSlotTime(endDate, mainSlot) || endDate, max_students: parseInt(max_students) || 50, description: description || null, zoom_link: zoom_link || null, time_slots: normalizedSlots, status: "PENDING_APPROVAL" } });
      await tx.scheduledSession.createMany({ data: liveItems.map((s, idx) => ({ batch_id: newBatch.id, session_number: s.session_number, name: s.name, type: s.type || "CAD", scheduled_at: withSlotTime(sessionDates[idx], mainSlot), status: "UPCOMING" })) });
      return newBatch;
    });
    const fullBatch = await prisma.batch.findUnique({ where: { id: batch.id }, include: { course: { select: { name: true } }, scheduled_sessions: { orderBy: { session_number: "asc" } }, _count: { select: { scheduled_sessions: true, enrollments: true } } } });
    return success(res, 201, `Batch "${batchName}" created with ${totalSessions} live sessions scheduled. ${recordedProjects.length} recorded project(s) will unlock with the first live project. Awaiting admin approval.`, { ...fullBatch, recorded_projects: recordedProjects });
  } catch (err) { next(err); }
};
...
const getMyBatches = async (req, res, next) => {
  try {
    ...
    const batches = await prisma.batch.findMany({ where: { tutor_id: req.user.id, ...(status && { status }) }, orderBy: { start_date: "desc" }, include: { course: { select: { name: true, slug: true } }, _count: { select: { enrollments: true, scheduled_sessions: true } } } });
...
const updateFullBatch = async (req, res, next) => {
  try {
    ...
    const tutorId = req.user.id;
    const existingBatch = await prisma.batch.findFirst({ where: { id: batchId, tutor_id: tutorId }, include: { scheduled_sessions: { orderBy: { session_number: "asc" }, include: { _count: { select: { assignments: true, queries: true } } } } } });
    if (!existingBatch) return error(res, 404, "Batch not found.");
    const { name, start_date, end_date, max_students, description, status, time_slots, sessions } = req.body;
    const updateData = {};
    ...
    if (time_slots !== undefined) {
      const normalizedSlots = Array.isArray(time_slots) ? time_slots.map((slot) => String(slot || "").trim()).filter(Boolean) : typeof time_slots === "string" && time_slots.trim() ? [time_slots.trim()] : existingBatch.time_slots || [];
      ...
      if (incomingSessions) {
        ...
        for (const oldSession of currentSessions) {
          if (!incomingIds.has(oldSession.id)) {
            if ((oldSession._count?.assignments || 0) > 0 || (oldSession._count?.queries || 0) > 0) await tx.scheduledSession.update({ where: { id: oldSession.id }, data: { status: "CANCELLED" } });
            else await tx.scheduledSession.delete({ where: { id: oldSession.id } });
          }
        }
        for (let index = 0; index < incomingSessions.length; index += 1) {
          const item = incomingSessions[index] || {};
          const sessionNumber = index + 1;
          const sessionData = { session_number: sessionNumber, name: String(item.name || "").trim() || `Session ${sessionNumber}`, type: safeSessionType(item.type), scheduled_at: item.scheduled_at ? new Date(item.scheduled_at) : null, status: safeSessionStatus(item.status) };
          if (item.id && currentById.has(item.id)) await tx.scheduledSession.update({ where: { id: item.id }, data: sessionData });
          else await tx.scheduledSession.create({ data: { batch_id: batchId, ...sessionData } });
        }
      }
      return tx.batch.findUnique({ where: { id: batchId }, include: { course: { select: { name: true, slug: true } }, tutor: { select: { name: true } }, scheduled_sessions: { orderBy: { session_number: "asc" }, include: { assignments: { select: { id: true, title: true, due_date: true } } } }, assignments: { orderBy: { created_at: "asc" } }, _count: { select: { enrollments: true, scheduled_sessions: true } } } });
    });
      ...
      include: {
        batch: {
          include: {
            ...
            assignments: { select: { id: true, submissions: { where: { student_id: studentId }, select: { id: true, status: true, submitted_at: true } } } },
            _count: { select: { scheduled_sessions: true, assignments: true } },
          },
        },
      },
    });
    const projectMap = await getTutorCourseProjectMap(enrollments.map((e) => ({ tutor_id: e.batch?.tutor_id, course_id: e.batch?.course_id })));
...
const getBatchDetails = async (req, res, next) => {
  try {
    ...
    const batch = await prisma.batch.findUnique({ where: { id: req.params.id }, include: { course: { select: { name: true, slug: true, tools_covered: true } }, tutor: { select: { name: true } }, scheduled_sessions: { orderBy: { session_number: "asc" }, include: { assignments: { select: { id: true, title: true, due_date: true } } } }, assignments: { orderBy: { created_at: "asc" } } } });
    if (!batch) return error(res, 404, "Batch not found.");
    if (role === "STUDENT") {
    ...
    } else if (role === "TUTOR" && batch.tutor_id !== userId) return error(res, 403, "You do not own this batch.");
    const projectMap = await getTutorCourseProjectMap([{ tutor_id: batch.tutor_id, course_id: batch.course_id }]);
    return success(res, 200, "Batch details.", { ...batch, recorded_projects: projectMap.get(`${batch.tutor_id}:${batch.course_id}`) || [] });
  } catch (err) { next(err); }
};
...
const getCourseBatchesForRegistration = async (req, res, next) => {
  try {
    ...
    const batches = await prisma.batch.findMany({ where: { course_id: req.params.courseId, status: { in: ["APPROVED", "UPCOMING", "ACTIVE"] }, start_date: { gte: minDate, lte: maxDate } }, orderBy: { start_date: "asc" }, select: { id: true, name: true, start_date: true, end_date: true, created_at: true, status: true, max_students: true, time_slots: true, tutor: { select: { name: true } }, _count: { select: { enrollments: true } } } });
    const batchIds = batches.map((b) => b.id);
    let offerMap = new Map();
    if (batchIds.length > 0) {
      try {
        const offerRows = await prisma.$queryRaw`SELECT id, offer_name, original_price, offer_price, offer_start_at, offer_end_at FROM batches WHERE id = ANY(${batchIds})`;
        offerMap = new Map(offerRows.map((row) => [row.id, row]));
      } catch { offerMap = new Map(); }
    }
    const result = batches.map((b) => {
      ...
      return { id: b.id, name: b.name, start_date: b.start_date, end_date: b.end_date, created_at: b.created_at, status: b.status, tutor_name: b.tutor?.name || "DCT Tutor", time_slots: b.time_slots || [], enrolled, available_seats: availableSeats, is_full: availableSeats <= 0, offer_name: offer.offer_name || null, original_price: offer.original_price || null, offer_price: offer.offer_price || null, offer_start_at: offer.offer_start_at || null, offer_end_at: offer.offer_end_at || null };
    }).filter((b) => !b.is_full);
    return success(res, 200, "Available batches.", result);
  } catch (err) { next(err); }
};

module.exports = { createBatch, getMyBatches, updateBatch, updateFullBatch, getEnrolledBatches, getBatchDetails, getCourseBatchesForRegistration };
