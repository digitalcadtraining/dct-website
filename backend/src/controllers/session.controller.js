/**
 * Session + Assignment + Query Controller
 * Deploy-ready: real sessions, submissions, reviews, student queries.
 */
...
const updateSession = async (req, res, next) => {
  try {
    ...
    const updated = await prisma.scheduledSession.update({ where: { id: req.params.id }, data: updateData });
      ...
      const assignments = await prisma.assignment.findMany({
        ...
        include: {
          ...
          submissions: req.user.role === "STUDENT"
            ...
            : { select: { id: true, status: true, grade: true, feedback: true, file_url: true, submitted_at: true, reviewed_at: true, student: { select: { id: true, name: true, email: true } } } },
        },
      });
  ...
  submitAssignment: async (req, res, next) => {
    try {
      ...
      const submission = await prisma.assignmentSubmission.upsert({
        ...
        update: { file_url: req.file.path, status: "SUBMITTED", submitted_at: new Date(), reviewed_at: null },
      });
  ...
  getTutorSubmissions: async (req, res, next) => {
    try {
      ...
      const submissions = await prisma.assignmentSubmission.findMany({
        where: {
          assignment: {
            batch: { tutor_id: req.user.id, ...(batch_id && { id: batch_id }) },
  ...
  getMyQueries: async (req, res, next) => {
    try {
      const queries = await prisma.query.findMany({
        ...
        include: { session: { select: { id: true, session_number: true, name: true } } },
      });
  ...
  getBatchQueries: async (req, res, next) => {
    try {
      ...
      const queries = await prisma.query.findMany({
        ...
        orderBy: { created_at: "desc" },
        include: { student: { select: { name: true } }, session: { select: { id: true, session_number: true, name: true } } },
      });
