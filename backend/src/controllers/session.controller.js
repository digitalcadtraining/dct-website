...
const getBatchSessions = async (req, res, next) => {
  try {
    ...
    if (req.user.role === "STUDENT") {
      const enrolled = await prisma.enrollment.findFirst({ where: { student_id: req.user.id, batch_id: batchId } });
      if (!enrolled) return error(res, 403, "You are not enrolled in this batch.");
    } else if (req.user.role === "TUTOR") {
      const owns = await prisma.batch.findFirst({ where: { id: batchId, tutor_id: req.user.id } });
    ...
    const sessions = await prisma.scheduledSession.findMany({
      ...
      include: {
        assignments: { select: { id: true, title: true, description: true, due_date: true, file_url: true } },
        queries: req.user.role === "STUDENT"
          ? { where: { student_id: req.user.id }, select: { id: true, question: true, attachment_url: true, answer: true, status: true, created_at: true, answered_at: true } }
          : false,
      },
    });
...
const updateSession = async (req, res, next) => {
  try {
    ...
    if (session.batch.tutor_id !== req.user.id) return error(res, 403, "You do not own this session.");
    ...
    const updated = await prisma.scheduledSession.update({ where: { id: req.params.id }, data });
...
const assignmentController = {
  createAssignment: async (req, res, next) => {
    try {
      ...
      if (!batch_id || !title) return error(res, 400, "Batch and assignment title are required.");
      ...
      if (session_id) {
        const session = await prisma.scheduledSession.findFirst({ where: { id: session_id, batch_id } });
      ...
      const assignment = await prisma.assignment.create({
        data: {
          ...
          file_url: cleanUploadPath(req.file),
          due_date: due_date ? new Date(due_date) : null,
        },
      });
      return success(res, 201, "Assignment uploaded successfully.", assignment);
    } catch (err) { next(err); }
  },
  ...
  getBatchAssignments: async (req, res, next) => {
    try {
      ...
      const assignments = await prisma.assignment.findMany({
        ...
        include: {
          session: { select: { id: true, session_number: true, name: true } },
          submissions: req.user.role === "STUDENT"
            ? { where: { student_id: req.user.id }, select: { id: true, status: true, grade: true, feedback: true, file_url: true, submitted_at: true, reviewed_at: true } }
            : { select: { id: true, status: true, grade: true, feedback: true, file_url: true, submitted_at: true, reviewed_at: true, student: { select: { id: true, name: true, email: true, phone: true } } } },
        },
      });
  ...
  submitAssignment: async (req, res, next) => {
    try {
      ...
      const enrolled = await prisma.enrollment.findFirst({ where: { student_id: req.user.id, batch_id: assignment.batch_id } });
      if (!enrolled) return error(res, 403, "Not enrolled in this batch.");
      if (!req.file) return error(res, 400, "Please upload your assignment file.");

      const file_url = cleanUploadPath(req.file);
      const submission = await prisma.assignmentSubmission.upsert({
        where: { assignment_id_student_id: { assignment_id: req.params.id, student_id: req.user.id } },
        create: { assignment_id: req.params.id, student_id: req.user.id, file_url, status: "SUBMITTED" },
        update: { file_url, status: "SUBMITTED", submitted_at: new Date(), reviewed_at: null, feedback: null, grade: null },
      });
  ...
  getTutorSubmissions: async (req, res, next) => {
    try {
      ...
      const submissions = await prisma.assignmentSubmission.findMany({
        where: { assignment: { batch: { tutor_id: req.user.id, ...(batch_id && { id: batch_id }) }, ...(session_id && { session_id }) } },
        orderBy: { submitted_at: "desc" },
        include: { student: { select: { id: true, name: true, email: true, phone: true } }, assignment: { include: { batch: { select: { id: true, name: true } }, session: { select: { id: true, session_number: true, name: true } } } } },
      });
      return success(res, 200, "Tutor submissions fetched.", submissions);
    } catch (err) { next(err); }
  },
  ...
  reviewSubmission: async (req, res, next) => {
    try {
      ...
      const submission = await prisma.assignmentSubmission.findUnique({ where: { id: req.params.id }, include: { assignment: { include: { batch: { select: { tutor_id: true } } } } } });
      ...
      const updated = await prisma.assignmentSubmission.update({ where: { id: req.params.id }, data: { grade: grade || null, feedback: feedback || null, status: status || "REVIEWED", reviewed_at: new Date() } });
...
const queryController = {
  createQuery: async (req, res, next) => {
    try {
      ...
      if (!batch_id || !question) return error(res, 400, "Batch and question are required.");
      ...
      const query = await prisma.query.create({
        ...
        include: {
          session: { select: { id: true, session_number: true, name: true, scheduled_at: true } },
  ...
  answerQuery: async (req, res, next) => {
    try {
      ...
      if (req.user.role === "TUTOR") {
        ...
        if (!batch) return error(res, 403, "Not authorized to answer this query.");
      }

      const updated = await prisma.query.update({
        where: { id: req.params.id },
        data: { answer: answer.trim(), status: status || "RESOLVED", answered_at: new Date() },
      });
