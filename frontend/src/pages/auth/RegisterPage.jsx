          ...
          <AnimatePresence mode="wait">
            {step === 1 && (
              <motion.div key="step1"
                ...
                <div className="mb-5 rounded-2xl border border-blue-100 bg-blue-50 p-4 text-sm text-dct-gray">
                  <p className="font-bold text-dct-primary">Registration Fee: ₹{REGISTRATION_FEE}</p>
                  <p className="mt-1">Your dashboard access starts only after WhatsApp OTP verification and successful payment.</p>
                </div>
                    ...
                    <motion.div initial={{ opacity:0, height:0 }} animate={{ opacity:1, height:"auto" }}>
                      ...
                      {form.batch_id && selectedBatch && (
                        <div className="mt-2 p-3 rounded-xl bg-blue-50 border border-blue-100 text-xs text-dct-gray space-y-1">
                          <p><strong>Starts:</strong> {new Date(selectedBatch.start_date).toLocaleDateString("en-IN", { day:"numeric", month:"long", year:"numeric" })}</p>
                          {selectedBatch.time_slots?.length > 0 && <p><strong>Timing:</strong> {selectedBatch.time_slots.join(", ")}</p>}
                          <p><strong>Tutor:</strong> {selectedBatch.tutor_name || "Industry Expert"}</p>
                        </div>
                      )}
                    </motion.div>
                  )}
            ...
            {step === 3 && (
              <motion.div key="step3"
                ...
                <p className="text-sm text-dct-lightgray text-center mb-6">
                  Pay registration fee to activate your student dashboard.
                </p>
                ...
                <div className="rounded-3xl border border-blue-100 bg-gradient-to-br from-blue-50 to-white p-5 shadow-sm space-y-4">
                  <div className="flex items-center justify-between border-b border-blue-100 pb-4">
                    <div>
                      <p className="text-xs uppercase tracking-wider text-dct-gray font-bold">Registration Fee</p>
                      <p className="text-4xl font-black text-dct-primary">₹{REGISTRATION_FEE}</p>
                    </div>
                    <div className="h-14 w-14 rounded-2xl bg-dct-primary text-white grid place-items-center font-black">D</div>
                  </div>

                  <div className="text-sm text-dct-gray space-y-2">
                    <p><strong>Student:</strong> {form.name}</p>
                    <p><strong>Course:</strong> {selectedCourse?.name || "Selected course"}</p>
                    <p><strong>Batch:</strong> {selectedBatch?.name || "Selected batch"}</p>
                    <p><strong>Phone:</strong> +91 {form.phone}</p>
                  </div>
                ...
                <Button fullWidth size="lg" onClick={startPayment} disabled={loading} className="mt-5">
                  {loading ? "Opening Payment…" : "Pay ₹999 & Register"}
                </Button>
              </motion.div>
            )}
          </AnimatePresence>
