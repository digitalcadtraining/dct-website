...
export default function RegisterPage() {
  ...
  const applyDiscount = async () => {
    ...
    if (!form.course_id || !form.batch_id)
      return setDiscountMsg("Select course and batch first.");
    setDiscountLoading(true);
    try {
      const res = await discountCodeApi.validate({
        code,
        course_id: form.course_id,
        batch_id: form.batch_id,
        current_price: baseCoursePrice,
      });
      setAppliedDiscount(res.data);
      setDiscountMsg(
        `Code applied. Final course fee ₹${getDisplayPrice(res.data.final_price)}.`,
      );
    } catch (e) {
            ...
            {step === 1 && (
              ...
              >
                ...
                <PriceBox
                  ...
                  originalPrice={selectedBatchOriginalPrice}
                  registrationFee={REGISTRATION_FEE}
                  appliedDiscount={appliedDiscount}
                  firstEmiDate={firstEmiDate}
                  secondEmiDate={secondEmiDate}
                />
                ...
                <div className="rounded-3xl border border-blue-100 bg-gradient-to-br from-blue-50 to-white p-5 shadow-sm space-y-4">
                  <div className="flex items-center justify-between border-b border-blue-100 pb-4">
                    <div>
                      ...
                      <p className="text-xs text-dct-gray mt-1">
                        Course fee locked: ₹{getDisplayPrice(payableAmount)}
                      </p>
                    </div>
                  ...
                  <div className="text-sm text-dct-gray space-y-2">
                    ...
                    <p>
                      ...
                      {getDisplayPrice(
                        Math.ceil(
                          Math.max(0, payableAmount - REGISTRATION_FEE) / 2,
                        ),
                      )}{" "}
                      · {fmtDate(firstEmiDate)}
                    </p>
                    <p>
                      <strong>Second EMI:</strong> ₹
                      {getDisplayPrice(
                        Math.floor(
                          Math.max(0, payableAmount - REGISTRATION_FEE) / 2,
                        ),
                  ...
                  <div className="rounded-2xl bg-white border border-blue-100 p-3 text-xs text-dct-gray">
                    After Instamojo confirms ₹999 registration payment, your
                    account will be created automatically and you will get
                    dashboard access.
                  </div>
                </div>
                {err && (
    ...
    <div>
      ...
      <div className="flex gap-2">
        <input
          ...
          placeholder="Paste discount code"
          value={value}
        ...
        <button
          ...
          onClick={onApply}
          disabled={loading}
          className="px-4 rounded-xl bg-dct-primary text-white text-sm font-bold disabled:opacity-60"
        >
          {loading ? "Checking…" : applied ? "Applied" : "Apply"}
        </button>
      </div>
      {message && (
        <p
          className={`text-xs mt-1 font-semibold ${applied ? "text-green-700" : "text-dct-gray"}`}
        >
          {message}
        </p>
      )}
