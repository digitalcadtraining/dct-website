...
function toSessionDateInput(value, fallbackSlot) {
  if (!value) return "";

  const d = new Date(value);
  if (Number.isNaN(d.getTime())) return "";

  const slotStart = parseSlotStart(fallbackSlot);
  const utcTime = `${String(d.getUTCHours()).padStart(2, "0")}:${String(
    d.getUTCMinutes(),
  ).padStart(2, "0")}`;

  // Fix old corrupted rows saved as India time but treated as UTC
  if (slotStart && utcTime === slotStart) {
...
function NewBatchModal({ isOpen, onClose, onCreated }) {
  ...
  useEffect(() => {
    ...
    tutorApi
      ...
      .then((r) => {
        ...
        setCourses(list);
        if (list.length === 1)
          setForm((f) => ({ ...f, course_id: list[0].id }));
      })
      .catch((e) => {
  ...
  const submit = async () => {
    setErr("");
    if (!form.course_id)
      return setErr("No approved course found for your tutor account.");
    if (!form.start_date) return setErr("Select start date.");
    if (!form.start_time || !form.end_time)
      return setErr("Select start and end time.");
    if (form.start_time >= form.end_time)
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        <div className="px-6 py-5 border-b border-gray-100 flex justify-between">
          <div>
            ...
            <p className="text-xs text-dct-lightgray">
              Only your approved course is shown
            </p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          <div>
            ...
            {courses.length === 0 && !err && (
              <p className="text-xs text-orange-600 mt-2">
                No approved course found. Ask admin to approve your tutor
                application first.
              </p>
            )}
          </div>
          ...
          <div>
            <label className="block text-xs font-bold text-dct-gray mb-2 uppercase">
              Class Timing *
            </label>
            ...
            {slot && (
              <span className="mt-3 inline-flex items-center gap-2 text-xs font-bold px-3 py-2 rounded-full border border-blue-200 bg-blue-50 text-dct-primary">
                <Clock size={12} />
                {slot}
              </span>
            )}
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              type="button"
              onClick={() => set("alt_days", !form.alt_days)}
              className={`p-3 rounded-xl border-2 text-left ${form.alt_days ? "border-dct-primary bg-blue-50" : "border-gray-200"}`}
            >
              <p className="text-xs font-bold">Alternate Days</p>
              <p className="text-[10px] text-dct-lightgray">Every 2 days</p>
            </button>
            <button
              type="button"
              onClick={() => set("sunday_off", !form.sunday_off)}
              className={`p-3 rounded-xl border-2 text-left ${form.sunday_off ? "border-dct-primary bg-blue-50" : "border-gray-200"}`}
            >
              <p className="text-xs font-bold">Sunday Off</p>
        ...
        <div className="px-6 py-4 border-t">
          <button
            ...
            className="w-full py-3.5 rounded-xl text-white text-sm font-bold disabled:opacity-60 bg-dct-primary"
          >
            {loading ? "Creating..." : "Submit for Admin Approval →"}
          </button>
        </div>
      </motion.div>
    </div>
...
function EditFullBatchModal({ batch, onClose, onSaved }) {
      ...
      .then((r) => {
        ...
        setForm({
          ...
          sessions: (b.scheduled_sessions || []).map((s, index) => ({
            ...
            name: s.name || "",
            type: s.type || "BOTH",
            date: toSessionDateInput(s.scheduled_at, b.time_slots?.[0]),
  ...
  const setSession = (i, k, v) =>
    setForm((f) => ({
      ...f,
      sessions: f.sessions.map((s, idx) => (idx === i ? { ...s, [k]: v } : s)),
    }));
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-6xl z-10 max-h-[94vh] flex flex-col overflow-hidden">
        <div className="px-7 py-5 border-b flex items-start justify-between">
          <div>
            ...
            <p className="text-xs text-dct-gray mt-1">
              Edit batch details and session-wise syllabus. Students see updates
              automatically.
            </p>
            {form?.start_time && form?.end_time && (
              <p className="text-xs font-bold text-dct-primary mt-2">
                Batch slot: {makeSlot(form.start_time, form.end_time)}
              </p>
            )}
          </div>
        ...
        {loading && (
          <div className="p-10 text-center text-sm text-dct-gray">
            Loading batch...
          </div>
        )}
          ...
          <>
            <div className="flex-1 overflow-y-auto px-7 py-6 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">
                  ...
                  <textarea
                    rows={3}
                    className="dct-input resize-none"
              ...
              <div className="border-t pt-5">
                <div className="flex items-center justify-between gap-3 mb-4">
                  <div>
                    ...
                    <p className="text-xs text-dct-gray">
                      Default time uses batch slot. Change session time only for
                      exceptions.
                    </p>
                  </div>
                  <button
                    type="button"
                    onClick={addSession}
                    className="px-4 py-2.5 rounded-xl bg-dct-primary text-white text-xs font-bold flex items-center gap-2"
                  >
                ...
                <div className="space-y-3">
                  {form.sessions.map((s, i) => (
                    <div
                      key={s.id || `new-${i}`}
                      className="rounded-2xl border p-4 bg-gray-50/60"
                    >
                      <div className="flex items-center justify-between mb-3">
                        <p className="text-xs font-extrabold text-dct-primary">
                          Session {i + 1}
                        </p>
                        <button
                          type="button"
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-[2fr_1fr_1fr_1fr_1fr] gap-3">
                        <div>
                          <label className="block text-[10px] font-bold text-dct-gray mb-1 uppercase">
                            Topic Name
                          </label>
                          <input
                            className="dct-input"
                            value={s.name}
                            onChange={(e) =>
                              setSession(i, "name", e.target.value)
                            }
                          />
                        </div>
                        <div>
                          ...
                          <select
                            ...
                            value={s.type}
                            onChange={(e) =>
                              setSession(i, "type", e.target.value)
                            }
                          >
                        ...
                        <div>
                          ...
                          <input
                            ...
                            value={s.date}
                            onChange={(e) =>
                              setSession(i, "date", e.target.value)
                            }
                          />
                        </div>
                        <div>
                          ...
                          <input
                            ...
                            value={s.time}
                            onChange={(e) =>
                              setSession(i, "time", e.target.value)
                            }
                          />
                        </div>
                        <div>
                          ...
                          <select
                            ...
                            value={s.status}
                            onChange={(e) =>
                              setSession(i, "status", e.target.value)
                            }
                          >
                            <option value="UPCOMING">Upcoming</option>
            ...
            <div className="px-7 py-4 border-t bg-white flex justify-end gap-3">
              ...
              <button
                onClick={save}
                disabled={saving}
                className="px-6 py-3 rounded-xl bg-dct-primary text-white text-sm font-bold disabled:opacity-60 flex items-center gap-2"
              >
                <Save size={15} /> {saving ? "Saving..." : "Save Full Batch"}
              </button>
            </div>
          </>
        )}
...
function BatchCard({ batch, index, onEdit }) {
  ...
  return (
    <motion.div
      className="bg-white rounded-2xl border border-gray-100 p-5 hover:shadow-lg hover:border-dct-primary/30 transition-all"
    ...
    >
      ...
      <h3 className="font-extrabold text-dct-dark text-sm mb-0.5 leading-snug">
        {batch.name}
      </h3>
      ...
      {slots.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mb-3">
          {slots.map((s) => (
            <span
              key={s}
              className="flex items-center gap-1 text-[10px] font-semibold px-2 py-1 rounded-full bg-blue-50 text-dct-primary"
            >
              <Clock size={9} />
              {s}
            </span>
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        <div className="border rounded-lg p-2.5">
          ...
          <p className="text-[10px] text-dct-lightgray">Start</p>
        </div>
        <div className="border rounded-lg p-2.5">
          <p className="text-xs font-bold">
            {batch.end_date
          ...
          <p className="text-[10px] text-dct-lightgray">End</p>
        </div>
      </div>
      ...
      <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden mb-4">
        <div className="h-full bg-dct-primary" style={{ width: `${pct}%` }} />
      </div>
      <div className="flex items-center justify-between pt-3 border-t gap-2">
        <button
          type="button"
          onClick={() => nav(`/tutor/batches/${batch.id}/sessions`)}
        ...
        <button
          ...
          className="text-xs font-bold text-dct-primary border border-dct-primary rounded-lg px-3 py-1.5 flex items-center gap-1"
        >
          <Edit3 size={13} />
          Edit Batch
        </button>
...
export default function MyBatches() {
  ...
  const list =
    filter === "ALL" ? batches : batches.filter((b) => statusOf(b) === filter);
      ...
      <PageWrapper>
        <div className="flex items-center justify-between mb-6 gap-3 flex-wrap">
          ...
          <div className="flex gap-2">
            <button
              onClick={load}
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl border bg-white text-sm font-bold"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-white text-sm font-bold bg-dct-primary"
            >
              <Plus size={16} />
              New Batch
            </button>
        ...
        <div className="flex gap-2 mb-6 flex-wrap">
          {[
            ...
            <button
              key={k}
              onClick={() => setFilter(k)}
              className={`px-4 py-2 rounded-xl text-xs font-semibold ${filter === k ? "bg-dct-primary text-white" : "bg-gray-100 text-dct-gray"}`}
            >
              {l}
            </button>
          ))}
        ...
        {loading && (
          <div className="text-center py-16 text-dct-gray text-sm">
            Loading...
          </div>
        )}
        {!loading && list.length === 0 && (
          <div className="bg-white rounded-2xl p-16 text-center border">
            ...
            <p className="font-bold">No batches</p>
          </div>
        )}
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
          {list.map((b, i) => (
            <BatchCard key={b.id} batch={b} index={i} onEdit={setEditBatch} />
          ))}
        </div>
