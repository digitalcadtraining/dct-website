...
function toTimeInput(d, fallback) {
  ...
  const x = new Date(d);
  if (x.getUTCHours() === 0 && x.getUTCMinutes() === 0)
    return parseSlotStart(fallback);
...
function Badge({ s }) {
  ...
  return (
    <span
      className={`text-[10px] font-bold px-2.5 py-1 rounded-full ${m[s] || m.UPCOMING}`}
    >
      {s}
    </span>
  );
  ...
  const save = async () => {
    ...
    try {
      await sessionApi.update(session.id, {
        [completed ? "recording_url" : "zoom_link"]: url.trim(),
      });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl p-6 max-w-md w-full z-10">
        <h3 className="font-extrabold mb-3">
          {completed ? "Add Recording" : "Add Class Link"}
        </h3>
        <p className="text-sm text-dct-gray mb-3">
          Session {session.session_number}: {session.name}
        </p>
        <input
          className="dct-input mb-4"
          value={url}
        ...
        <button
          ...
          className="w-full py-3 rounded-xl bg-dct-primary text-white font-bold disabled:opacity-50"
        >
...
function EditSessionModal({ session, batch, onClose, onSaved }) {
  ...
  const [time, setTime] = useState(
    toTimeInput(session.scheduled_at, batch?.time_slots?.[0]),
  );
  ...
  const save = async () => {
    ...
    try {
      const scheduled_at = combineDateTime(
        date,
        time || parseSlotStart(batch?.time_slots?.[0]),
      );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-black/50" onClick={onClose} />
      <div className="relative bg-white rounded-2xl p-6 max-w-md w-full z-10">
        <h3 className="font-extrabold mb-1">Edit Session Date & Timing</h3>
        <p className="text-sm text-dct-gray mb-4">
          Session {session.session_number}: {session.name}
        </p>
        <div className="grid grid-cols-2 gap-3 mb-4">
          <div>
            <label className="block text-xs font-bold text-dct-gray mb-1 uppercase">
              Date
            </label>
            <input
              type="date"
        ...
        <div className="mb-4">
          ...
          <p className="text-[11px] text-dct-lightgray mt-1">
            Student side updates automatically.
          </p>
        </div>
        <button
          onClick={save}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl p-6 max-w-md w-full z-10">
        ...
        <div className="bg-blue-50 rounded-xl p-3 text-sm mb-3">
          {query.question}
        </div>
        <textarea
          rows={4}
          className="dct-input resize-none mb-4"
          value={answer}
...
export default function TutorSessionsPage() {
  ...
  const enriched = useMemo(
    () => sessions.map((s) => ({ ...s, _status: statusOf(s) })),
    [sessions],
  );
  const shown =
    filter === "ALL"
      ...
      <PageWrapper>
        <div className="flex items-center justify-between gap-3 mb-5">
          <div>
            ...
            <p className="text-sm text-dct-gray">
              Edit individual session date/time, add links, create assignments
              and answer doubts
            </p>
          </div>
          <div className="flex gap-2 flex-wrap justify-end">
            <button
              onClick={() => nav("/tutor/batches")}
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-dct-primary text-white text-sm font-bold"
            >
              <Edit3 size={15} />
              Edit Batch
            </button>
            <button
              onClick={loadData}
              className="flex items-center gap-2 px-4 py-2 rounded-xl border bg-white text-sm font-bold"
            >
          ...
          <div className="flex gap-3 overflow-x-auto pb-2 mb-5">
            {batches.map((b) => (
              <button
                ...
                onClick={() => {
                  setSelected(b);
                  nav(`/tutor/batches/${b.id}/sessions`);
                }}
                className={`min-w-[220px] text-left rounded-2xl border p-4 ${selected?.id === b.id ? "border-dct-primary bg-blue-50" : "bg-white"}`}
              >
                <p className="text-xs font-bold text-dct-primary mb-1">
                  {b.status}
                </p>
                <p className="text-sm font-extrabold text-dct-dark line-clamp-2">
                  {b.name}
                </p>
                <p className="text-xs text-dct-lightgray mt-1">
                  {b.time_slots?.[0] || "No time"}
                </p>
              </button>
            ))}
          </div>
            ...
            <div className="flex gap-2 mb-5 flex-wrap">
              {["ALL", "TODAY", "UPCOMING", "COMPLETED"].map((f) => (
                <button
                  key={f}
                  onClick={() => setFilter(f)}
                  className={`px-4 py-2 rounded-xl text-xs font-bold ${filter === f ? "bg-dct-primary text-white" : "bg-gray-100 text-dct-gray"}`}
                >
                  {f}
                </button>
              ))}
            ...
            <div className="grid grid-cols-1 xl:grid-cols-[1fr_320px] gap-5">
              <div className="space-y-3">
                    ...
                    >
                      ...
                      <div className="grid grid-cols-2 gap-3 my-4">
                        ...
                        <div className="border rounded-xl p-3">
                          <Clock size={14} />
                          <p className="text-xs font-bold mt-1">
                            {fmtTime(s.scheduled_at, selected.time_slots?.[0])}
                          </p>
                        </div>
                      </div>
                      <div className="grid grid-cols-3 gap-2">
                        <button
                          onClick={() => setEdit(s)}
                          className="py-2.5 rounded-xl border border-dct-primary text-dct-primary text-xs font-bold flex items-center justify-center gap-1"
                        >
                          <Edit3 size={13} />
                          Edit
                        </button>
                        <button
                          onClick={() => setLink(s)}
                          className="py-2.5 rounded-xl bg-dct-primary text-white text-xs font-bold flex items-center justify-center gap-1"
                        >
                          <LinkIcon size={13} />
                          Link
                        </button>
                        <button
                          onClick={() =>
                            nav(
                              `/tutor/assignments?batch_id=${selected.id}&session_id=${s.id}`,
                            )
                          }
                          className="py-2.5 rounded-xl border border-dct-primary text-dct-primary text-xs font-bold flex items-center justify-center gap-1"
                        >
                ...
                {!loading && shown.length === 0 && (
                  <div className="bg-white rounded-2xl border p-12 text-center text-dct-gray">
              ...
              <div className="bg-white rounded-2xl border p-4 h-fit">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-xs font-bold bg-red-50 text-red-600 px-2 rounded-full">
                    {openQueries.length}
                  </span>
                </div>
                <div className="space-y-3">
                  {openQueries.slice(0, 8).map((q) => (
                    <div key={q.id} className="border rounded-xl p-3">
                      ...
                      <p className="text-xs text-dct-gray line-clamp-2 my-2">
                        {q.question}
                      </p>
                      <button
                        onClick={() => setAnswer(q)}
                        className="text-xs font-bold text-white bg-dct-primary px-3 py-1.5 rounded-lg"
                      >
                        Answer
                      </button>
                    </div>
                  ...
                  {openQueries.length === 0 && (
                    <p className="text-sm text-dct-lightgray text-center py-8">
                      No open queries
                    </p>
                  )}
                </div>
        ...
        {link && (
          <LinkModal
            ...
            onSaved={(id, url, completed) =>
              setSessions((v) =>
                v.map((s) =>
                  s.id === id
                    ? { ...s, [completed ? "recording_url" : "zoom_link"]: url }
                    : s,
                ),
              )
            }
        ...
        {answer && (
          <AnswerModal
            ...
            onSaved={(id) =>
              setQueries((q) =>
                q.map((x) => (x.id === id ? { ...x, status: "RESOLVED" } : x)),
              )
            }
          />
        )}
