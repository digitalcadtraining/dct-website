...
function initInterior360Hero(add) {
  ...
  const hotspots = Array.from(document.querySelectorAll(".interior360-hotspot"));
  ...
  function next() { updateFrame(current + 1); }
  function prev() { updateFrame(current - 1); }
  ...
  add(prevBtn, "click", () => { userTouched = true; stopAuto(); prev(); });
  add(nextBtn, "click", () => { userTouched = true; stopAuto(); next(); });
...
function initHeroAutoExplorer(add) {
  ...
  const parts = {
    plastic: ["instrumentPanel", "doorTrim", "centerConsole", "bPillarTrim", "frontBumper"],
