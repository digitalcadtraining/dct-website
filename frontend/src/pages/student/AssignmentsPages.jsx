...
function SubmitModal({ assignment, onClose, onDone }) {
  ...
  useEffect(() => {
    // Every assignment must start with its own clean upload state.
    // Prevents a file selected for one assignment from carrying
    // into another assignment's Replace/Submit modal.
    setFile(null);
    setErr("");
  }, [assignment?.id]);
  ...
  const submit = async () => {
    ...
    try {
      ...
      setErr(error.message || "Could not submit the assignment.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    ...
    >
      <div className="space-y-4">
        <Input label="Session Assignment" value={assignment.title} readOnly />
...
function AskModal({ assignment, batchId, onClose }) {
  ...
  return (
    <Modal isOpen={Boolean(assignment)} onClose={onClose} title="Ask Doubt">
      <div className="space-y-4">
        <Input label="Session Assignment" value={assignment.title} readOnly />
        <Textarea
          label="Question"
...
function AssignmentCard({ assignment, onSubmit, onAsk, onFeedback }) {
  const submission = subOf(assignment);
  const reviewed = submission?.status === "REVIEWED";
  const resubmit = submission?.status === "RESUBMIT";
  const awaitingReview = submission?.is_locked && !reviewed && !resubmit;
  ...
  return (
    <div className="dct-card p-5">
      <div className="flex justify-between gap-2">
        <div>
          <h3 className="font-extrabold text-dct-dark">{assignment.title}</h3>
      ...
      <PageWrapper>
        ...
        <div className="flex items-center justify-between gap-3 mb-4">
          <div>
            ...
            <p className="text-xs text-dct-gray mt-1">
              A submission task opens automatically after each live session is
              completed. You can replace your file for 48 hours.
            </p>
          </div>
        ...
        {!state.loading && state.assignments.length === 0 && (
          <div className="bg-white border rounded-2xl p-12 text-center">
            ...
            <p className="font-bold">No assignments are open yet</p>
            <p className="text-sm text-dct-gray mt-1">
              An assignment will appear automatically after a live session is
              marked completed.
            </p>
          </div>
        )}
        ...
        <SubmitModal
          key={submitAssignment?.id || "assignment-submit-modal"}
        ...
        >
          <div className="space-y-3">
            <Input
              label="Grade"
              value={subOf(feedbackAssignment)?.grade || "Not graded"}
              readOnly
            />
            <Textarea
              label="Tutor Feedback"
              value={subOf(feedbackAssignment)?.feedback || "No feedback"}
              readOnly
              rows={5}
            />
          </div>
